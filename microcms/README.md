...
ブログ機能で使う microCMS の準備手順。スキーマ定義はこのディレクトリの JSON をインポートする。
...
> インポートがエラーになる場合は、上記 JSON の `apiFields` の内容を見ながら手動でフィールドを作成しても同じ（5フィールドのみ）。
...
microCMS 管理画面 → `blogs` API →「API設定」→「Webhook」→「カスタム通知」
...
- **コンテンツIDがURLになる**（`/blog/{コンテンツID}`）。作成時に `my-first-post` のような英数字スラッグへ書き換えること
